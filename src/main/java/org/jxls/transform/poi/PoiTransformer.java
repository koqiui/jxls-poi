 ...
 *
 * @author Leonid Vysochyn Date: 1/23/12
 * @author koqiui addImage changed to support image
 */
public class PoiTransformer extends AbstractTransformer {
	...
	private void copyMergedRegions(CellData sourceCellData, CellRef destCell) {
		...
		if (cellMergedRegion != null) {
			...
			destSheet.addMergedRegion(new CellRangeAddress(destCell.getRow(), destCell.getRow() + cellMergedRegion.getLastRow() - cellMergedRegion.getFirstRow(), destCell.getCol(),
					destCell.getCol() + cellMergedRegion.getLastColumn() - cellMergedRegion.getFirstColumn()));
		}
	}
	...
	public void addImage(AreaRef areaRef, byte[] imageBytes, ImageType imageType) {
		addImage(areaRef, imageBytes, imageType, null, null);
	}
